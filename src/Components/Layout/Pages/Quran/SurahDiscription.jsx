...
import { FaBookReader, FaNotesMedical } from 'react-icons/fa';
import { FaPause, FaPlay } from 'react-icons/fa6';
import { IoBookOutline, IoPlay } from 'react-icons/io5';
import { LuBadgeInfo, LuBookText } from 'react-icons/lu';
import { useNavigate, useParams } from 'react-router-dom';
import apiClient from './apiClient';

function SurahDiscription() {
  const navigate = useNavigate();
  const [surahDetails, setSurahDetails] = useState([]);
  const { id } = useParams();
  const [active, setActive] = useState('Recitation');
  ...
  // Fetch surah data and filter first ayah for Surah 1
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await apiClient.get(`surah/${id}`);
        let ayahs = res.data.data.ayahs;

        // Remove first ayah for Surah 1 (Al-Fatiha)
        if (id == 1) {
          ayahs = ayahs.slice(1);
        }

        setSurahDetails(ayahs);
      } catch (err) {
  ...
  // Scroll handler for page detection
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;

    const handleScroll = () => {
      const pageSections = container.getElementsByClassName('page-section');
      ...
      Array.from(pageSections).forEach((section) => {
        ...
        if (distance < minDistance) {
          ...
          closestPage = parseInt(section.dataset.page);
        }
      });
  ...
  // Icons array
  const icons = [
    { id: 1, icon: <IoBookOutline className="hover:text-white" /> },
    { id: 2, icon: <IoPlay className="hover:text-white" /> },
    { id: 3, icon: <FaNotesMedical className="hover:text-white" /> },
    { id: 4, icon: <BsThreeDots className="hover:text-white" /> },
  ];

  return (
    <div className="md:mt-20 mt-16 w-full pt-10">
      {/* Toggle between Recitation and Translation */}
      <div className="flex justify-center">
        <div className="flex bg-[#343A40] rounded-3xl p-1 justify-center">
          <button
            onClick={() => setActive('Translation')}
            className={`flex maven px-7 py-2 w-1/2 rounded-3xl gap-2 justify-center transition-all duration-500 items-center ${
              active === 'Translation' ? 'bg-[#202424] text-white' : ''
            }`}
          >
          ...
          <button
            onClick={() => setActive('Recitation')}
            className={`flex maven px-6 py-2 w-1/2 rounded-3xl gap-3 justify-center items-center ${
              active === 'Recitation' ? 'bg-[#202424] text-white' : ''
            }`}
          >
            <FaBookReader className="text-md surah-name" />
            Recitation
          </button>
        </div>
      </div>
      ...
      {/* Surah Content */}
      <div className="flex flex-col items-center w-full px-4 py-10 border-b-[.00001px]">
        {/* Surah Name */}
        <h1 className="quran-text font-extrabold text-3xl border-b pb-2 text-white mb-5 text-center">
          {convertToArabicNumbers(id)}
        </h1>

        {/* Surah Info and Audio Controls */}
        <div className="flex gap-32 justify-between">
          <h2
            onClick={() => navigate(`/Quran/${id}/info`)}
            className="flex cursor-pointer transform transition-all duration-100 active:scale-90 hover:bg-[#48d3e536] rounded px-2 items-center gap-2 maven text-lg text-gray-300 my-5"
          >
            {active === 'Recitation' && (
              <>
                <LuBadgeInfo />
          ...
          <h2
            onClick={playAudio}
            className="flex px-2 rounded transform transition-all duration-100 active:scale-90 hover:bg-[#48d3e536] cursor-pointer items-center justify-center gap-2 maven text-lg text-gray-300 my-5"
          >
            {active === 'Recitation' &&
              (isPlaying ? (
                <>
        ...
        {/* Recitation Section */}
        {active === 'Recitation' && (
          <div className="flex  flex-col items-center  text-xl leading-loose w-full md:w-2/3 lg:w-1/2 xl:w-[40%] text-center text-white  py-6">
            <div
              ref={containerRef}
              className="w-full h-[75vh]  overflow-y-auto scroll-smooth"
            >
              {/* Bismillah design for every sorah */}
              <p className="flex bg-black/30 justify-evenly border-[.1px] border-[#4A5565] items-center mb-5 pb-2">
                <img
                  src="/quran.png"
                  alt=""
              ...
              {[...new Set(surahDetails.map((ayah) => ayah.page))].map(
                (pageNumber) => {
                  const pageVerses = surahDetails.filter(
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`text-2xl px-2 bg-black/20 text-justify text-align-last tracking-wider leading-loose quran-UTMNI text-gray-100`}
                      >
                        {pageVerses.map((ayah) => (
                          <span className="" key={ayah.number}>
                            <span className="">{ayah.text}</span>
                            <span className="text-2xl text-cyan-300 mx-2">
                              {convertToArabicNumbers(ayah.numberInSurah)}
                            </span>
                          </span>
                        ))}
                      </div>
                      {/* Page Separator */}
                      {
                        <div className="mt-10 mb-10 border-b-3 border-dotted border-gray-600 relative">
                          <span className="absolute left-1/2 -translate-x-1/2 -top-3 px-2 bg-gray-900 text-gray-400 text-lg">
                            Page {convertToArabicNumbers(pageNumber)}
                          </span>
                        </div>
                      }
                    </div>
        ...
        {/* Audio Element */}
        <audio ref={AudioRef}>
          <source src="/childQuran.mp3" type="audio/mpeg" />
        ...
        {active === 'Translation' && (
          <>
            <div className="flex justify-center sm:flex-row flex-col sm:justify-between w-full px-0 sm:px-10 md:px-20 lg:px-32 maven">
              <div>
                <label htmlFor="Translation">Translate to: </label>
                <select
                  className="border-b pb-1 px-5 outline-none maven"
                  name="Lang"
                  value={selectedLang}
                ...
                >
                  ...
                  <option value="Farcy">Farcy</option>
                </select>
              </div>
              <div className="flex sm:flex-col flex-row gap-x-4 mt-5 sm:mt-0 justify-between sm:gap-y-2 maven cursor-pointer">
            ...
            <div className="px-0 sm:px-10 md:px-20 lg:px-32 w-full">
              {surahDetails.map((verse) => (
                <div
                  key={verse.id}
                  className="text-white border-b pt-10 pb-5 flex flex-col lg:flex-row gap-y-5 lg:gap-y-0 justify-between relative"
                >
                  <div className="flex flex-col items-center lg:items-start space-y-4 text-gray-400">
                    <div className="flex items-center text-start flex-row lg:flex-col space-x-3 lg:space-x-0 lg:space-y-4 p-2">
                      <span className="maven p-2">
                        {verse.numberInSurah}:{id}
                      </span>
                      {icons.map((item) => (
                  ...
                  <div className="w-full flex flex-col md:gap-y-0 gap-y-5 justify-between">
                    <div className="text-end">
                      <span>
                        <span className="text-xl leading-loose quran-UTMNI ">
                          {verse.text}
                        </span>
                        <span className="quran-UTMNI text-cyan-300 mx-2">
                          <span>
                            {' '}
                            {convertToArabicNumbers(verse.numberInSurah)}{' '}
                          </span>
                        </span>
                      </span>
                    </div>

                    <div className="maven lg:text-start mb-5">
                      ...
                      <p className="text-sm flex items-center mt-3 sm:ml-3 text-[#70757a]">
                        <LuBookText className="text-md" />
                        <span>
                          . Translate by
                          <span className="underline text-[#2BA4AB]">
                            {' '}
                            dash{' '}
                          </span>
                        </span>
...
export default SurahDiscription;
